...
to setup
  ...
  set world-features create-species
  ...
  [
    ...
    set agent-world-score calculate-score species world-features
  ]
...
to go
  ...
  let total-agent-world-score sum [agent-world-score] of turtles

  remove-extinct-species
  set current-species-list []
  set agent-count-list insert-item 0 agent-count-list count turtles
  ; if the population is stable or a number of time steps has passed, mutate an individual in the populaion
  ...
  [
    ...
    ifelse coin-flip [right random 90][left random 90] ; randomly goes either right or left by 90 degrees
    forward 1
    ...
    carefully
    [
      set energy energy + (world-resources * (agent-world-score / total-agent-world-score))
    ]
    []
    ...
    [
      ;set pcolor red
      ;print [species] of one-of other turtles-here
      let other-agent one-of other turtles-here
      let other-species [species] of other-agent ;one-of other turtles-here
    ...
    ;set energy energy - 2 ;Reduces the amount of energy by a constant amount
    if energy <= 0 [die]
...
to reproduce [species1 energy-level]
  ; Has an X% chance of randomly mutating a reproduced individual
...
to-report find-connections
  ...
  let food-web-connections find-world-connections world-features
  loop
  [
    ...
    let ignore-species? count turtles with [species = current-species] ;;;
    if score-difference < competition-parameter and not member? connection-array food-web-connections and ignore-species? > 5
    [
      let color-array []
      set color-array insert-item 0 color-array find-turtle-color item j shortened-species-list
      set color-array insert-item 0 color-array find-turtle-color current-species
      set food-web-connections insert-item 0 food-web-connections connection-array
    ]

    set j j + 1

    set food-web-connections remove-duplicates food-web-connections ; Removes any duplicates that may exist
...
; Finds the species that feed on the world
to-report find-world-connections [world]
  let food-web-connections []
  let i 0
  loop
  [
    let s item i current-species-list
    let score calculate-score s world
    let ignore-species? count turtles with [species = s]
    if score > 0 and ignore-species? > 5
    [
      let connection []
      set connection insert-item 0 connection world
      set connection insert-item 0 connection s
      set food-web-connections insert-item 0 food-web-connections connection
    ]
    set i i + 1
    if i >= length current-species-list [report food-web-connections]
  ]
end

; Find a list of predators
to-report find-predators
  let connections find-world-connections world-features
  let i (length current-species-list) - (length connections)
  report i
end
...
695
396
The amount of energy added to the world in each time step
11
0.0
1
