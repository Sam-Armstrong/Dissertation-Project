...
to go
  ...
  [
    ...
    [
      ...
      if predatory-ability * (random-float 2) > [defence] of other-agent and [species] of other-agent != species
      [
    ...
    [
      ...
      if plant-eating-ability * (random-float 2) > p-defence
      [
...
to reproduce [current-agent]
  ...
  [
    ...
    [
      ...
      set speed big-mutation [speed] of current-agent
      set metabolism (predatory-ability + plant-eating-ability + (defence) + (speed / 5))
  ...
  [
    ...
    [
      ...
      set speed mutate [speed] of current-agent
      set metabolism (predatory-ability + plant-eating-ability + (defence) + (speed / 5))
...
to-report find-species
  ...
  carefully
  [
    let max-pred [predatory-ability] of max-one-of turtles [predatory-ability]
    let max-plant [plant-eating-ability] of max-one-of turtles [plant-eating-ability]
    let max-defence [defence] of max-one-of turtles [defence]
    let max-speed [speed] of max-one-of turtles [speed]
    ...
    [
      ...
      ifelse predatory-ability > plant-eating-ability
      [
        if predatory-ability <= max-pred / 4 [set pr-a 0]
        if predatory-ability > max-pred / 4 and predatory-ability <= max-pred / 2 [set pr-a 1]
        if predatory-ability > max-pred / 2 and predatory-ability <= (3 * max-pred) / 4 [set pr-a 2]
        if predatory-ability > (3 * max-pred) / 4 and predatory-ability <= max-pred [set pr-a 3]
        ;if predatory-ability > max-pred [set pr-a 4]

        set pl-a 0
      ]
      [
        if plant-eating-ability <= max-plant / 4 [set pl-a 0]
        if plant-eating-ability > max-plant / 4 and plant-eating-ability <= max-plant / 2 [set pl-a 1]
        if plant-eating-ability > max-plant / 2 and plant-eating-ability <= (3 * max-plant) / 4 [set pl-a 2]
        if plant-eating-ability > (3 * max-plant) / 4 and plant-eating-ability <= max-plant [set pl-a 3]

        set pr-a 0
      ]

      let def 0
      if defence <= max-defence / 4 [set def 0]
      if defence > max-defence / 4 and defence <= max-defence / 2 [set def 1]
      if defence > max-defence / 2 and defence <= (3 * max-defence) / 4 [set def 2]
      if defence > (3 * max-defence) / 4 and defence <= max-defence [set def 3]

      let spe 1

      set species []
      set species insert-item 0 species spe
      set species insert-item 0 species def
      set species insert-item 0 species pl-a
      set species insert-item 0 species pr-a

      if not member? species species-list
      [
        set species-list insert-item 0 species-list species
      ]
    ]
  ]
  []
...
to-report find-connections
  ...
  loop
  [
    set shortened-species-list remove i species-list
    ...
    if item 0 current-species > item 2 item j shortened-species-list * 1.0 and item 0 current-species > item 1 current-species and current-species != item j shortened-species-list
    [
    ...
    set food-web-connections remove-duplicates food-web-connections ;Provides redundancy by removing any duplicates that may exist

    if j >= length shortened-species-list - 1
    [
      if i >= length species-list - 1
      [
        report food-web-connections
      ]
      set i i + 1
      set current-species item i species-list
      set j 0
    ]
  ]
end
...
to-report find-world-connections
  ...
  ask turtles
  [
    if predatory-ability <= plant-eating-ability ;predatory-ability - average-pred-ability < plant-eating-ability - average-plant-ability
    [
      if plant-eating-ability > 0.3 and not member? species used-species
      [
        let connection []
        set connection insert-item 0 connection "Plant 1"
        set connection insert-item 0 connection species
        set food-web-connections insert-item 0 food-web-connections connection
      ]

      if plant-eating-ability > 0.7 and not member? species used-species
      [
        let connection []
      ...
      if plant-eating-ability > 1 and not member? species used-species
      [
        let connection []
      ...
      if plant-eating-ability > 1.3 and not member? species used-species
      [
        let connection []
    ...
    set used-species insert-item 0 used-species species
  ]
